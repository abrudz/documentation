...
`⎕JSON` supports six variant options, `Format`, `Dialect`, `Null`, `Compact`, `Charset`, and `HighRank`, specified using the _variant_ operator [`⍠`](../primitive-operators/variant.md), summarised in [](#variantoptionsforjson), and described in detail beneath it. The principal option is `Format`. The three options that affect only export are tolerated here, but have no effect.
