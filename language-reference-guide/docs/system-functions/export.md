...
The left domain of `⎕EXPORT` might be extended in future to include extra types (for example, to change the behaviour of the function). This means that, although `⎕EXPORT` returns a Boolean result in the first version, this might not be the case in the future. If you need a Boolean result, use `0≠` or an equivalent.
```apl
