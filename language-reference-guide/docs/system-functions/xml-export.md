...
`R` is a character vector containing the generated XML. Character references and entity references are emitted in place of characters where necessary, to ensure that valid XML is generated. Only the predefined entity references are used.

A [CDATA section](xml.md#cdata-sections) can be generated by presenting it as markup, which is how one survives a round trip: [importing](xml-import.md#limitations) turns its content into character data.

Markup, if present, is *not* validated, so it is possible to generate invalid XML if care is not taken with markup constructs.
